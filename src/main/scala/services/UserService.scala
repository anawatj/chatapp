...
class UserService(userRepository: UserRepository,uuidUtil:UUIDUtils,passwordUtil:PasswordUtils) {
          ...
          userRepository.findByUserName(loginRequest.username.getOrElse("")) map {
            case Some(user)=>
                passwordUtil.isHash(loginRequest.password.getOrElse(""),user.password) match {
                  case true=>Right(UserResponse(UserResponseData(user.id,user.username,user.password,user.first_name,user.last_name),StatusCodes.OK.intValue))
